...
@router.post("/item/{item_id}/link/{target_id}", status_code=status.HTTP_201_CREATED)
async def create_user_link(
    ...
    """Create a manual USER_LINK edge between two items."""
    from app.models.edge import Edge
    from app.models.item import Item
    from sqlalchemy import select

    # Verify both items exist and belong to user
    for iid in (item_id, target_id):
        item = (
            await db.execute(
                select(Item).where(
                    Item.id == iid,
    ...
    # Sync to Neo4j
    if settings.use_neo4j_graph:
        await neo4j_client.create_user_link(str(item_id), str(target_id), note)
...
async def remove_user_link(
    ...
    """Delete a manual USER_LINK edge."""
    if settings.use_neo4j_graph:
